...
if ( ! function_exists( 'simple_setup' ) ) :
	...
	function simple_setup() {
		...
		register_nav_menus( array(
			'header-bottom' => esc_html__( 'Navigation menu at bottom of header', 'simple' ),
		) );
...
function simple_scripts() {
	...
	wp_enqueue_script( 'bootstrap', get_template_directory_uri() . '/js/bootstrap.min.js', array(), '20200213', true );
...
function simple_get_nav_login_link_text(){
 /**
  * Returns appropriate login text
  */
  $user = wp_get_current_user();
  return ( $user->ID === 0 ) ? 'Sign In' : 'Hello, <b>' . ucfirst($user->display_name) . '</b>';
